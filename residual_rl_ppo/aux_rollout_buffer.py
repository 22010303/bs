"""
aux_rollout_buffer.py — PPO rollout buffer with auxiliary targets.
"""
...
class PPOAuxRolloutBuffer:
    ...
    def reset(self):
        ...
        self.next_force_targets = np.zeros((c, self.ft_dim), dtype=np.float32)
        self.state_labels = np.zeros((c,), dtype=np.int64)
    ...
    def add(
        ...
        self.next_force_targets[self.ptr] = next_force_target
        self.state_labels[self.ptr] = state_label
        self.ptr += 1
    ...
    def get(self, device='cpu'):
        ...
        return {
            ...
            'next_force_target': torch.FloatTensor(self.next_force_targets[:size]).to(device),
            'state_label': torch.LongTensor(self.state_labels[:size]).to(device),
        }
