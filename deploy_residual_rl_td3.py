"""
deploy_residual_rl_td3.py — Deploy SmolVLA + trained residual TD3 policy.
"""
...
def parse_args():
    p = argparse.ArgumentParser(description='Deploy SmolVLA + Residual TD3')
...
def main():
    ...
    ckpt_path = os.path.join(args.rl_checkpoint, 'td3_checkpoint.pt')
    ckpt = torch.load(ckpt_path, map_location=device, weights_only=False)
    cfg = ckpt.get('config', ResidualTD3Config())
    ...
    trainer = TD3Trainer(config=cfg, device=str(device))
    ...
    while pih_env.env.is_viewer_alive():
        ...
        if pih_env.env.loop_every(HZ=20):
            ...
            if contact:
                cart_residual = trainer.select_action(obs['state'], obs['ft_history'], deterministic=True)
            ...
            phase = "TD3_ACTIVE" if contact else "SMOLVLA_ONLY"
