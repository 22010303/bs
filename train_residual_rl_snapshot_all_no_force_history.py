"""
train_residual_rl_snapshot_all_no_force_history.py — Snapshot-based residual RL
trainer that supports SAC / PPO / TD3 in a single script while keeping
instantaneous force feedback but removing force history from policy input.

Compared with train_residual_rl_snapshot_all.py:
  - Current calibrated force/torque is still included in the state.
  - The temporal force-history branch is disabled by feeding zeros.
  - Reward, force-based early termination, snapshot reset, and SmolVLA base
    policy remain aligned with the current architecture.
    python train_residual_rl_snapshot_all_no_force_history.py --algo ppo --snapshot ./ckpt/residual_rl/snapshot_step315.pkl
"""
...
def build_rl_obs_no_force_history(pih, ft_offset, cfg):
    ft = pih.get_force_torque() - ft_offset
    ee = pih.get_ee_pose()
    pt = pih.env.get_p_site(PEG_TIP_SITE)
    he = pih.env.get_p_site(HOLE_ENTRY_SITE)
    rel = (he - pt).astype(np.float32)
    state_vec = np.concatenate([ft, ee, rel], dtype=np.float32)
...
class TD3Adapter(AlgoAdapter):
    ...
    def log_episode(self, ep_reward, ep_len, ep_info, reward_info):
        ...
        if reward_info:
            ...
            for k in [
                'r_force_mag', 'r_stability', 'r_depth', 'r_progress',
...
ALGO_REGISTRY = {
    'sac': (SACAdapter, ResidualRLConfig, './ckpt/residual_rl_snapshot_no_force_history'),
    'ppo': (PPOAdapter, ResidualPPOConfig, './ckpt/residual_rl_snapshot_ppo_no_force_history'),
    'td3': (TD3Adapter, ResidualTD3Config, './ckpt/residual_rl_snapshot_td3_no_force_history'),
}
...
def main():
    ...
    if use_wandb:
        ...
        run_config.update({
            ...
            'force_feedback_input': True,
            'force_history_input': False,
        })
        wandb.init(
            project=args.wandb_project,
            name=args.wandb_name or f'residual_rl_snap_{args.algo}_no_force_history_seed{cfg.seed}',
            config=run_config,
        )
    ...
    def reset_from_snapshot():
        ...
        print(
            f"\n--- Episode {episode} --- "
            f"dp={info['dp']*1000}mm drpy={np.rad2deg(info['drpy'])}deg  ft_cal={ft_cal}"
        )
    ...
    def store_terminal(done_reward, is_success):
        ...
        ft = pih.get_force_torque() - ft_offset
        ft_history.append(np.zeros(cfg.ft_dim, dtype=np.float32))
        state_vec, ft_hist = build_rl_obs_no_force_history(pih, ft_offset, cfg)
        pt = pih.env.get_p_site(PEG_TIP_SITE)
        he = pih.env.get_p_site(HOLE_ENTRY_SITE)
        hb = pih.env.get_p_site(HOLE_BOTTOM_SITE)
        reward, rinfo = reward_fn.compute(ft, pt, he, hb, is_success)
        reward += done_reward
    ...
    print(f"\n=== Snapshot Residual RL ({args.algo.upper()}, NO FORCE HISTORY) ===")
    ...
    while pih.env.is_viewer_alive() and adapter.total_steps < cfg.total_steps:
        ...
        if not contact_detected and check_contact(pih, ft_offset):
        ...
        if contact_detected and cfg.max_residual_pos > 0:
            ...
            if prev_obs is not None:
                ...
                he = pih.env.get_p_site(HOLE_ENTRY_SITE)
                hb = pih.env.get_p_site(HOLE_BOTTOM_SITE)
                reward, rinfo = reward_fn.compute(ft, pt, he, hb, False)
                episode_reward += reward
                last_rinfo = rinfo
                adapter.store_step(
                    state=prev_obs[0],
        ...
        if contact_detected and prev_obs is not None:
            pih.env.viewer_text_overlay(
                text1=f'Residual {args.algo.upper()} (no force history)',
                text2=(
